...
In finance, an **option** is: 

* an agreement, which gives the holder a **right** 
* to **buy** or to **sell** 
* a certain underlying instrument 
* for a given price (*exercise price*) 
* at a given moment in futures (*expiry date*).

Since the option offers a right, not an obligation, the holder can decide whether to exercise the option or not.

That right has its price - the price of the option, also called **option premium**.
...
library(RQuantLib)
(price <- 
    EuropeanOption(type = "call", underlying = 110, strike = X,
...
S <- seq(90, 140, 1)
longCallPayoffs <- map_dbl(S, \(S) max(S - X, 0))
tibble(x = S, y = longCallPayoffs - price) |> 
  ggplot(aes(x = x, y = y)) + 
...
```{r, echo = F, warning=F}
S <- seq(80, 130, 1)
longPutPayoffs <- map_dbl(S, \(S) max(X - S, 0))
tibble(x = S, y = longPutPayoffs - price) |> 
  ggplot(aes(x = x, y = y)) + 
  geom_line(linewidth = 1, col = "red") +
  ylim(-10, 20) + theme_minimal() +
  labs(subtitle = paste0("Black-Scholes premium, X = ", X,
...
```{r, echo = F, warning=F}
tibble(x = S, y = price - longPutPayoffs) |> 
  ggplot(aes(x = x, y = y)) + 
  geom_line(linewidth = 1, col = "red") +
  ylim(-20, 10) + theme_minimal() +
  labs(subtitle = paste0("Black-Scholes premium, X = ", X,
...
dt <- read_csv(here("data/quotes_2025-12-01.csv"))
dt |> 
  filter(expiration == "2025-12-19") |> 
  filter(option_type == "C") |> 
  mutate(strike = strike |> as.factor()) |> 
  ggplot(aes(quote_datetime, mid, color = strike)) +
  geom_line() +
  labs(title = "Mid-quotes of call options for S&P 500", 
...
dt |> 
  filter(expiration == "2025-12-19") |> 
  filter(option_type == "P") |> 
  mutate(strike = strike |> as.factor()) |> 
  ggplot(aes(quote_datetime, mid, color = strike)) +
  geom_line() +
  labs(title = "Mid-quotes of put options for S&P 500", 
       subtitle = "quotes for 2025-12-01, expiration = 2025-12-29",
       x = "", y = "") +
  theme_minimal()
```
...
dt |> 
  filter(strike == 6800) |> 
  filter(option_type == "C") |> 
  filter(between(expiration, as.Date("2025-12-02"), as.Date("2026-06-30"))) |> 
  mutate(expiration = expiration |> as.factor()) |> 
  ggplot(aes(quote_datetime, mid, color = expiration)) +
  geom_line() +
  labs(title = "Mid-quotes of call options for S&P 500", 
       subtitle = "quotes for 2025-12-01, strike = 6800",
       x = "", y = "") +
  theme_minimal()
```
...
The price $S_t$ at time $t$ starts from $S_0$ and follows 

$$S_t = S_0 \exp\left( \left( \mu - \frac{\sigma^2}{2} \right) t + \sigma W_t \right)$$
where $W_t$ is standard Brownian motion, $\mu$ is drift, and $\sigma$ is volatility. 

Thus, $$\ln(S_t / S_0) \sim \mathcal{N} \left( \left( \mu - \frac{\sigma^2}{2} \right) t, \sigma^2 t \right)$$

# Stock price motion in Monte Carlo methods

The probability density function:

$$f_{S_t}(s) = \frac{1}{s \sigma \sqrt{2 \pi t}} \exp \left( -\frac{ \left( \ln(s / S_0) - \left( \mu - \frac{\sigma^2}{2} \right) t \right)^2 }{ 2 \sigma^2 t } \right)$$ 

for $s > 0$.

Expected value: $$\mathbb{E}[S_t] = S_0 e^{\mu t}$$

Variance: $$\mathrm{Var}(S_t) = S_0^2 e^{2 \mu t} (e^{\sigma^2 t} - 1)$$. 
...
tibble(
  ...
  geom_line(linewidth = 0.25, aes(col = name)) +
  theme_minimal() +
